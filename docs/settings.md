...
| `cycleModels`          | array   | `[]`                        | Model patterns (same syntax as `enabledModels`, e.g. `provider/*`, `provider/id:high`) that Ctrl+P cycles instead of `cycleOrder`'s roles. Matched against the available models on every press, so models a provider's discovery adds or withdraws follow automatically. Empty keeps the role cycle.                                                                                                             |
