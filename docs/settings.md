...
| `cycleModels`          | array   | `[]`                        | Model patterns (same syntax as `enabledModels`, e.g. `provider/*`, `provider/id:high`) that Ctrl+P cycles instead of `cycleOrder`'s roles, matched against the available models on every press. For a models.yml `discovery` provider that has answered, only the models its listing names are cycled. Each press also re-asks those providers in the background (at most every 5 minutes; the first press of a session always asks), so an added or withdrawn model shows from a later press. Empty keeps the role cycle. |
...
| `advisor.holdNotesUntilTurnEnd` | boolean | `false` | Withhold `nit`/`concern` notes the advisor writes while the primary is mid-turn and deliver them together at the turn end. Off routes each admitted note as soon as it is written; blockers always route immediately. |
...
| `retry.usageLimitFallbackChains`         | record  | `{}`              | Same format as `retry.fallbackChains`, but consulted only when a turn fails on a usage limit (quota reached, Sprilicred `pool_exhausted`), ahead of the ordinary chains, for the main session and advisors. Nothing else reads it — not transient errors, startup model resolution, roles, subagent chain inheritance, or title models — so a reserve account such as `{"sprilicred-anthropic/*": ["personal-anthropic/*"]}` is spent only when the primary has run out. Keep the reserve provider out of `enabledModels` so it is never picked directly. |
...
| `display.timestamps`        | boolean | `false`          | Right-aligned start times and durations on transcript items; toggle with `/time`. |
| `display.timeZone`          | string  | `""`             | IANA zone for every clock omp shows and for today's date given to the model, e.g. `America/New_York`; empty uses the system zone. |
...
`display.timestamps` (or `/time [on|off|status]`) labels each transcript item at the right edge with the time it started: your messages, replies, tool calls (inside the right border of boxed cards such as bash), subagent calls, and notes such as compaction summaries. Once a step ends the label adds how long it took (`14:23:12 · 8.0s`); a running tool counts up in whole seconds. A reply's start sits on its first row and its duration on its last row, since the rows above may already be in scrollback; reply text that runs into a tool call ends where the call begins. Items from an earlier day include the date (`Sep 24 14:23:12`). Labels are display only and never sent to the model. After `--resume`, tool calls use their recorded start and end at their result; reply text followed by a tool call in the same message shows its start only, because the session does not record when it ended. `display.timeZone` accepts any IANA zone (daylight saving included) and applies to every clock omp shows: these labels, the usage row, the status line's `time` segment, the Agent Hub, `/btw` history, session lists, the git sidebar, the pinned `/usage` fetch time, stream chat, and the debug report. It also sets which day "today" is in the date/cwd reminder sent to the model. Worktree names, upload paths and dates in fetched web pages keep their own zone. An invalid zone is ignored with a warning.
...
`statusLine.preset: claude` reproduces the Claude Code statusline verbatim — `…/last/three/dirs [branch] | model (level) | ctx 62% | 5h 88% | wk 40%` (plus `| fable NN%` when a Fable-scoped weekly window is known) — with fixed 256-color foregrounds, bright-black `|` separators, and the terminal's own background. A detached HEAD shows the short commit id in place of the branch, as Claude Code's script does; a bare temp directory (omp's fallback for launches from `$HOME`: `/tmp`, `/var/tmp`, `~/tmp`, or macOS's `/private/tmp`) hides the whole path section, branch included, even when a lone repo clone sits inside it. When the session has a name — for example a `title.style: tag` tag — it follows as a final `| NAME` section. Gauges show the *remaining* share of each window and print `—` when the provider does not report one. Sprilicred-brokered sessions read the pool headline: each window's used share averaged across the pooled accounts that report it. Theme status-line colors, icons, separator, and `transparent` settings do not apply to this preset.
