...
By default tool calls are discarded rather than executed. `toolPolicy: "read-only"` (what `/btw` uses) instead runs pure lookups (`read`, `grep`, `glob`, `find`, `ast_grep`, `web_search`, `recall`) that the main agent currently has, on side-owned tool instances whose caches are separate from the main agent's, for at most 8 rounds; every other call gets an error result, nothing is appended to session history, and tool results go through the same secret obfuscation. `onToolCall` reports each call and whether it ran. Cursor-style transports that run tools server-side stay tool-free. Pass `tools: false` to remove tool definitions after context transforms and set `toolChoice: "none"` at the provider boundary. Omitting it preserves `/btw`'s tool catalog for prompt-cache reuse; disabling it may reduce cache hits. Existing context/provider hooks still run. It is not a sandbox or a guarantee that arbitrary extension hooks have no side effects. Model inference consumes the configured provider's resources. `dedupeReply` defaults to `true`: it collapses runs of 4+ identical lines and caps the reply at 4 KiB. Set it to `false` to retain the provider's exact text (`/btw` and `/omfg` do). Callers should use `replyText` for the final result.
