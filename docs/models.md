...
- `openai-models-list` reads each row's `id`, window (`max_model_len` or `context_length`), inputs (`input`/`input_modalities`) and, when a gateway sends them, `max_output_tokens` and `supports_reasoning`; advertised values win over the bundled catalog, so a model newer than the catalog can still think and write its full output. Models discovered for an `auth: oauth` provider are sent in the same OAuth request shape as its listed models.
