 ...
 * only accepted account selector — the durable stored-credential row id — and
 * is always resolved against the Codex provider; email/accountId are never
 * accepted so a mismatched pair cannot spend the wrong account's credit.
 * `redeemRequestId` is forwarded as the provider idempotency key so a client
 * retrying the same request cannot double-spend.
 */
