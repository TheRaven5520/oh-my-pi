...
- Added `ask.continueAfter`: after that many seconds without an answer, the agent continues on the recommended option (or its own judgment) instead of stalling, while the question stays open in the input box. A later answer is delivered to the agent with the assumption it worked on, so it can correct course; Esc tells it to keep the assumption. Other prompts go ahead of an open question, which then comes back unchanged. The turn ends normally while a question is open. Plan mode keeps waiting.
