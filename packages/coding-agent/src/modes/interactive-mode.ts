 ...
 * `rowOwners` overrides the positional mapping for the Pi-style dock, whose
 * rows (main root, above/below markers, hint) do not follow `order`.
 */
...
export class InteractiveMode implements InteractiveModeContext {
	...
	/**
	 * Sessions for the Pi-style dock: observer-registry snapshots plus any
	 * registry children (e.g. persisted workers re-registered on resume) the
	 * observer has not seen a frame for yet.
	 */
	...
	#renderSubagentList(): boolean {
		...
		if (lines.length > 0) {
			...
			// Row map for click-to-focus: blank, title, main root, optional
			// "above" marker, visible children, optional expander row.
	...
	/**
	 * Tear down like {@link shutdown}, then reopen this exact persisted session
	 * in a fresh process. Under the `launcher.ts` supervisor the relaunch is a
	 * handoff request (see {@link requestSupervisedRefresh}); direct `cli.ts`/SDK
	 * launches fall back to spawning the replacement and forwarding its exit code.
	 */
	async refresh(): Promise<void> {
		...
		try {
			...
			this.#handleTeardownError("refresh", error);
			return;
		}

		const cwd = this.sessionManager.getCwd();
		if (await requestSupervisedRefresh(invocation.sessionFile, cwd)) {
	...
	#handleTeardownError(action: "close" | "restart" | "refresh", error: unknown): void {
	...
	/** Shared `shutdown()`/`restart()`/`refresh()` teardown: dispose the session and hand the terminal back. */
