 ...
 * caller's abort signal, the turn's steering signal, and the user's explicit
 * background request. Returns the job's own completion when it settles first;
 * otherwise reports why the wait ended: "running" = threshold elapsed
 * (background it), "steer" = a queued message arrived mid-wait, "background" =
 * the user asked to move the work aside, "aborted" = the caller cancelled.
 */
