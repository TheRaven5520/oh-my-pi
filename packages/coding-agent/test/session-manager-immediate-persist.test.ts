...
describe("SessionManager JSONL software-crash durability", () => {
	...
	it("recovers an incomplete streaming checkpoint and discards it after normal completion", async () => {
		...
		const recovered = crashed.getEntries().find(entry => entry.type === "message" && entry.message.role === "assistant");
		expect(recovered?.type === "message" && recovered.message.role === "assistant" ? recovered.message.content : undefined).toEqual(
			newerPartial.content,
		);
		expect(recovered?.type === "message" && recovered.message.role === "assistant" ? recovered.message.stopReason : undefined).toBe(
			"aborted",
		);

		const complete = assistantMessage("completed answer");
	...
	it("keeps empty sessions out of history but retains accepted user intent", async () => {
		...
		expect(
			reopened.getEntries().some(
				entry => entry.type === "message" && entry.message.role === "user" && entry.message.content === "queued before assistant",
			),
		).toBe(true);
	});
