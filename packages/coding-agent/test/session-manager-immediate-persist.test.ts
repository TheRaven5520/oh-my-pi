...
describe("SessionManager JSONL software-crash durability", () => {
	...
	it("recovers an incomplete streaming checkpoint and discards it after normal completion", async () => {
		...
		// Checkpoints are keyed by message timestamp; a same-millisecond completion
		// would alias the partial answer instead of following it.
		const complete = { ...assistantMessage("completed answer"), timestamp: partial.timestamp + 1 };
