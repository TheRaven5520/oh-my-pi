		...
		await runInActiveSpan(toolSpan, async () => {
			try {
				...
				if (!completedToolExecution) {
					// The cooperative steering/background signals ride the loop-owned
					// ToolCallContext (surfacing as `ctx.toolCall.steeringSignal` and
					// `ctx.toolCall.backgroundSignal`): AgentToolContext itself is
					// app-built via declaration merging, so the loop cannot construct
					// or extend one structurally.
